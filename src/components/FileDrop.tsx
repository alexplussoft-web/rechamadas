...
type Props = {
  onText?: (text: string) => void; // opcional, caso queira receber texto
  onFileChange?: (e: ChangeEvent<HTMLInputElement>) => void; // mantém seu handleFile
};

export function FileDrop({ onText, onFileChange }: Props) {
  const [hover, setHover] = useState(false);
  const inputRef = useRef<HTMLInputElement | null>(null);

  const readFileAndEmitText = useCallback(
    (file: File | null) => {
      if (!file) return;
  ...
  const onFiles = useCallback(
    (file: File | null) => {
      ...
      readFileAndEmitText(file);
    },
    [onFileChange, readFileAndEmitText]
  );
